...
# Example: sc_fn = get_function('score', config)
...
def setup_run_folder(argv, config):
    ...
    os.system('cp {} {}/'.format(module_path, folder)) # copy the solver.py to the specific run folder 
...
def process(inp, out, solve_args, sc_fun):
    ...
    bsc = _get_best(testcase)  # get the best score for now 
    ...
    _save_ans(fname, folder, out)  # save answer output to runs/timestamp/fname.ans
    if sc > bsc:
        _save_ans(fname, 'ans', out) # save in ans/fname.ans
        _save_ans(testcase, 'submission', out) # save in submission/testcase.ans


    if sc > bsc:
        extra = ' BEST! Improved by: {}'.format(score2str(sc - bsc))
        logging.critical(now_sc_str + extra)
        _update_best(testcase, sc, folder)  # update max.json fields: score, folder (runs directory)
    else:
