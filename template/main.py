...
import glob # glob to recursively find files 
try:
...
def run_testcase(testcase, args):
    ...
    run_folder = setup_run_folder(sys.argv, config) # where to save run information
    init_log(testcase) 
    tear_down_streams = setup_stdstreams(run_folder)  # get the teardown function 
    ...
    def run(solve_args):
        ...
        process(inp, ans, solve_args_orig, sc_fn) # add to best submission or not
